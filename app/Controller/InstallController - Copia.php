...
class InstallController extends AppController{
  ...
  if (!$install->exists()) exit("You are not allowed to be here!");
      ...
      $conf = new File(APP."webroot/configuration.php");

      if ($conf->writable())
      {

        $sting = '<p class="success">app/webroot/configuration.php '.__('is writable').'</p>';
      ...
      else
      {
        ...
        $sting = '<p class="failed">app/webroot/configuration.php '.('is not writable').'</p>';
        $this->set('configuration', $sting);
